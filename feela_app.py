...
.chat-box { background: rgba(255,255,255,0.9); border-radius: 16px; padding: 20px; max-width: 780px; margin: 0 auto 30px; box-shadow: 0 8px 30px rgba(0,0,0,0.12); backdrop-filter: blur(6px); }
.chat-bubble-user { background: rgba(220,220,220,0.95); padding: 10px 14px; border-radius: 14px; margin: 10px 0; text-align: right; color: #333; }
.chat-bubble-bot { background: rgba(255,255,255,0.95); padding: 10px 14px; border-radius: 14px; margin: 10px 0; border: 1px solid rgba(247,207,161,0.6); color: #333; }
.title-chat { text-align: center; font-family: "Segoe UI", Tahoma, Geneva, Verdana, sans-serif; color: #0b63b8; text-shadow: 1px 1px 3px rgba(0,0,0,0.08); margin-bottom: 8px; }
.stButton>button { background-color: #1e90ff; color: white; border-radius: 10px; padding: 8px 18px; font-weight: 600; }
.stButton>button:hover { background-color: #0e5bb5; }
input[type="text"], input[type="password"], textarea { border-radius: 10px !important; padding: 10px !important; border: 1px solid #cfe8ff !important; }
</style>
"""
...
login_css = """
<style>
.login-container { min-height: 100vh; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #e6f3ff 0%, #ffffff 100%); font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
.login-card { background: linear-gradient(180deg, rgba(255,255,255,0.98), rgba(250,250,255,0.98)); padding: 36px 36px; border-radius: 16px; box-shadow: 0 12px 40px rgba(12, 60, 120, 0.12); max-width: 420px; width: 100%; text-align: center; }
.login-card .logo { font-size: 34px; margin-bottom: 10px; }
.login-card h1 { color: #0b63b8; margin-bottom: 10px; font-weight: 700; }
.login-sub { color: #4b6b8a; font-size: 14px; margin-bottom: 18px; }
.login-card input { width: 100%; padding: 12px; margin: 8px 0; border-radius: 10px; border: 1px solid #d9eefd; background-color: #fff; box-sizing: border-box; }
.login-card .muted { font-size: 13px; color: #6b7b8f; margin-top: 12px; }
.link-btn { background: none; border: none; color: #0b63b8; text-decoration: underline; cursor: pointer; font-weight: 600; padding: 0; }
</style>
"""
...
if not api_key:
    st.error(
        ...
        "Restart your terminal/IDE after running.\n"
        "Streamlit Cloud: Add key under Settings → Secrets."
    )
    st.stop()
...
def generate_feela_response(user_text):
    ...
    if mood == "sad":
        return f"Hey 💛, I sense you’re feeling low. Try {random.choice(exercises)} or {random.choice(snacks)}. You are precious!"
    elif mood == "happy":
        return random.choice(["Yay! That makes me happy 😄 too, Keep smiling!", "Awesome! 🌈 Spread those good vibes!", "Love that! 💖 Keep it going!"])
    elif mood == "angry":
        return f"It’s okay to feel angry 🔥. Try {random.choice(exercises)} to calm down. You are smart enough to handle this."

    # GPT fallback
    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[{"role": "system", "content": "You are Feela, a kind, casual, friendly mood companion chatbot."},
...
# MAIN APP FLOW
# -------------------------------
# Use new query_params instead of deprecated experimental_get_query_params
